...
export default {
  ...
  async created() {
    const DOMAIN = "/api/";
        ...
        this.blocCodeLists = newList.map(item => ({ text: item.blocNm, id:item.blocId}));
        if(this.searchOption){
          this.value = newList.map(item => ({ text: item.blocNm, id:item.blocId}))[0]
        }
        this.$emit('comm-cd-set', this.dataNm, this.dataTextNm, newList[0].blocId, newList[0].blocNm);
  },
  methods: {
    selectCommCd(val){
